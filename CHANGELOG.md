...
- **暗色电竞风格全面改版**：深蓝黑底 + Hextech 金强调，胜青绿/负绯红，我方冷蓝/敌方暖红；默认暗色优先，设置可切亮色
- **战绩页**：列表左色轨扫读、KDA/伤害/金钱/评分数字加大，相对条降噪；本人金边，MVP 金标 / ACE 银标；队头记分牌式呈现
- **对局页**：侦察卡以胜率/均 KDA 大数字为核心，自卡金轨，段位与评分统一金/冷蓝；近况行仅保留胜负色轨
- **应用壳**：侧栏 / 标题栏 / 滚动条随暗色主题适配；组件库迁移至 shadcn（Sidebar / Table / Dialog 等）

### 修复

- **设置页布局挤压**：窄列竖排标签、开关被裁切——容器补全宽度，改为「左文案 + 右控件」行式布局
- **浅色主题滚动条**：不再出现硬编码黑滚动条，随主题切换
- **顶部刷新范围**：只刷新当前页（战绩 / 对局），设置页禁用刷新
...
- **对局页长时间假死**：LCU 探测（最坏约 50 秒）不再持有全局锁——探测期间战绩/对局查询正常响应，退出不再被拖住
- **重赛（无效对局）判定不一致**：列表与详情统一判定口径，`teamEarlySurrendered` 不再漏判
- **战绩分页总数失真**：SGP 路径不再用「已扫描数」冒充总场数；总数未知时前端改为步进翻页
- **对局页刷新失败无提示**：失败原因现在可见（顶部错误条 + 日志）
- **单页崩溃拖垮全页**：三个页面各自隔离渲染错误

### 安全

- **更新链收紧**：更新源只信任 GitHub 官方端点（移除第三方镜像）；拒绝低于当前版本的「更新」（反回滚）
- **WebView CSP 收紧**；LCU 凭据 token 不再出现在任何调试输出 / 日志
- **SGP 请求参数转义补全**（query 元字符未编码的注入面关闭）

### 改进

- 段位显示不再做中文→英文往返转换；阶段文案单点维护
- 对局页「战绩数量」设置拆分为「战绩每页」与「近况场数」两个独立选项
- 更新弹窗支持键盘导航（焦点陷阱 / 焦点返还）；复制 Riot ID 等按钮键盘可达
- 日志带时间戳；应用退出时真正停止后台检测
- 内部契约命名修正（参团率 / 评分 / 查询键）与可空性建模统一（无感知）

### 工程

- 新增 README、`.editorconfig`、ESLint、前端单测（vitest）；CI 测试加 `--locked`
- 巨型文件拆分（gameinfo 2194→650 行等）；错误类型统一为可匹配枚举
